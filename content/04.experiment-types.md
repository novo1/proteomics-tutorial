...
A wide range of questions can be addressed using proteomics, translating to numerous workflows. 
Proteomics data can be both qualitative and quantitative. 
It may be desirable to identify what proteins are present and/or their absolute or relative abundance. 
Depending on the underlying goal(s), specific experimental strategies must be adapted. 
...
Several proteomics methods have been developed specific to each type of protein modification (see also the section "Protein/Peptide Enrichment and Depletion")

#### Phosphoproteomics
Phosphoproteomics is the study of the phosphorylation of proteins. 
Currently, there are two principal technologies employed to measure protein phosphorylation: mass spectrometry and SDS-PAGE/immunoblotting. 
A key challenge with phosphoproteomics is sensitivity. 
Despie being one of the most common PTMs observed in eukaryotic cells, at any one time, only about 1% of the proteome may be phosphorylated, necessitating specific phosphopeptide enrichment following sample trypsinisation.
Typically, this restriction comes at the cost of starting with a relatively high amount of input material for a phosphoproteomics project, typically around 1 mg of total protein per sample, but automation allows a drastic reduction down to 200 ug. 
...
Despite recent advances in phosphoproteomics workflows, multiple challenges still exist: limited sample amounts, highly complex samples, and an enormous dynamic range [@doi:10.1016/j.copbio.2008.06.006]. 
...
#### Glycoproteomics
Protein glycosylation, tzhe addition of carbohydrate chains of varying complexity to amino acid side chains, is gaining interest due to its ubiquity and emerging functional roles in cell and tissue biology. 
Protein  glycosylation occurs along the sectrtory pathway in the endoplasmic reticulum (ER) and Golgi apparatus and can be broadly classified into N (Asn)- and O (Ser/Thr)-linked glycosylation.
